...
/** 每个宿主创建独立动作运行时；服务调用与 Embed 策略不能成为核心的全局依赖。 */
export function createFormActionRuntime({ formActionRuntimeApi, uiEventBindingApi, createBusinessTraceKey, isEmbedDelegatedRequestEnabled = () => false }) {
async function resolveRuntimeFormActions(forms, context) {
 ...
 * 未来异步流程误清理后续动作状态。
 */
function acquireFormActionExecution(action, loadingState) {
 ...
 */
function buildCustomFormActionExecutionPayload(
  action,
  forms,
  context,
  requestId
...
function executeCustomFormAction(action, forms, context) {
