...
async function acceptForm(form) {
  ...
  try {
    ...
    const patched = await request(`/entity-forms/${form.id}/nodes/${selected.id}/patch`, {
      method: 'POST',
      body: JSON.stringify({
    ...
    await requestConflict(`/entity-forms/${form.id}/nodes/${selected.id}/patch`, {
      expectedRevision: selected.revision,
    ...
    await request(`/entity-forms/${form.id}/nodes/${selected.id}/patch`, {
      method: 'POST',
      body: JSON.stringify({
...
async function patchListField(listId, field, expectedRevision, fieldName) {
  return request(`/entity-list-config/${listId}/fields/${field.id}/patch`, {
    method: 'POST',
    body: JSON.stringify({
      expectedRevision,
      field: { fieldName }
...
async function patchListAction(listId, action, expectedRevision, buttonLabel) {
  return request(`/entity-list-config/${listId}/actions/${action.id}/patch`, {
    method: 'POST',
    body: JSON.stringify({
      expectedRevision,
      buttonLabel
...
async function acceptList(form) {
  ...
  try {
    ...
    await requestConflict(`/entity-list-config/${list.id}/actions/${action.id}/patch`, {
