...
const LOCATION_RULES = Object.freeze([
  ...
  locationRule('src/views/EntityListConfigDesign.vue', '^configInfo\\.(fixedFilterConfig|contextBindingConfig|queryProviderCode|queryDataSourceId)', '实体配置-列表-编辑-列表设置-查询实现'),
...
export const STRUCTURED_CONFIGURATIONS = Object.freeze([
  ...
  formNodeProperty('dataSource', '节点数据源', '绑定字段选项、默认值、计算、加载后或提交前数据源。', '{"usage":"FIELD_OPTIONS","sourceId":"10001"}', '运行时在对应阶段调用受控数据源并应用映射。', 'FIELD、SUB_FORM、REPEATER'),
...
const KEY_GUIDANCE = Object.freeze({
  ...
  queryDataSourceId: ['绑定统一数据源定义。', '10001', '列表通过已登记的数据源执行查询，并继承其权限与参数约束。'],
  ...
  sourceId: ['选择表单或节点绑定的统一数据源定义。', '10001', '运行时按发布版本调用该数据源并应用输入输出映射。'],
