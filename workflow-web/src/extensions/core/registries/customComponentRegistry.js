 ...
 * 自定义组件注册中心
 *
 * 开发者在 extensions/manifests 中声明 FORM/LIST；以下写入 API 仅由统一适配器调用。
 *
 * 自定义列表组件运行时契约 v2：
 *   - entityCode: 实体编码
...
/** 普通页面与关联内容的 props/runtime 不同，声明支持的宿主才能被选择和渲染。 */
function supportsUsage(descriptor, usageContext) {
  return Boolean(descriptor && (descriptor.usageContexts || ['PAGE']).includes(usageContext))
}
 ...
 */
export function getCustomListComponent(name, version, artifactDigest, usageContext = 'PAGE') {
  const descriptor = getCustomListDescriptor(name, version, artifactDigest)
  return supportsUsage(descriptor, usageContext) ? descriptor.component : undefined
}
 ...
 * @returns {boolean}
 */
export function hasCustomListComponent(name, version, artifactDigest, usageContext = 'PAGE') {
  return supportsUsage(getCustomListDescriptor(name, version, artifactDigest), usageContext)
}
...
export function getCustomListComponentOptions(usageContext = 'PAGE') {
  return getVersionedOptions(listRegistry)
    .filter(descriptor => supportsUsage(descriptor, usageContext))
    .map(({ component, ...descriptor }) => descriptor)
}

/** 返回指定列表组件所有已注册版本，供发布配置显式选版。 */
export function getCustomListComponentVersionOptions(name, usageContext = 'PAGE') {
 ...
 * @returns {Component|undefined}
 */
export function getCustomFormComponent(name, version, artifactDigest, usageContext = 'PAGE') {
  const descriptor = getCustomFormDescriptor(name, version, artifactDigest)
  return supportsUsage(descriptor, usageContext) ? descriptor.component : undefined
}
 ...
 * @returns {boolean}
 */
export function hasCustomFormComponent(name, version, artifactDigest, usageContext = 'PAGE') {
  return supportsUsage(getCustomFormDescriptor(name, version, artifactDigest), usageContext)
}
...
export function getCustomFormComponentOptions(usageContext = 'PAGE') {
...
export function getCustomFormComponentVersionOptions(name, usageContext = 'PAGE') {
