...
/&gt;</code></pre>
        ...
        <section id="data-source" class="guide-section">
          ...
          <ul class="check-list">
            ...
            <li>可选类型包括 `DICTIONARY`、`STATIC_OPTIONS`、`REGISTERED_PROVIDER`、`RUNTIME_CONTEXT`、`STRUCTURED_COMPUTE`。</li>
            <li>数据源统一配置输入/输出映射、分页、超时、缓存和失败策略，并在发布前按 Schema 校验。</li>
            <li>所有查询都必须执行平台传入的 `DataScopePlan`；Provider 不得返回授权范围外记录。</li>
            <li>禁止任意 SQL、脚本和外网 URL。外部系统访问只能封装在已注册 Provider 中，凭据由服务端安全管理。</li>
          </ul>
        </section>
