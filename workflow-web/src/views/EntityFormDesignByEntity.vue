...
import { createFormNodeEditorModel } from './form-designer/formNodeEditorModel.js'
...
import { buildRelationEditor } from '@/shared/relation-content'
...
import { getFormFieldValidationCapabilities, getFormNodeDataSourceUsages, getFormNodePropertySchema, normalizeFormFieldValidation, resolveFormNodeBinding, resolveFormNodeLayoutSpan } from '@flow/workflow-core/form-node-property-schema'
import {
...
import { isParentEntityReferenceTarget, isPublishedSubListOption, isSubListTargetFieldWritable, normalizeSubListParameterContract } from '@/shared/sub-list'
import {
  FORM_ACTION_MODES,
  emptyFormActionBar,
  footerFormActions,
...
} = createFormNodeEditorModel({ entityFields, activeExtensionMap })
...
async function loadRelatedContents() {
  ...
  try {
    ...
    relatedContents.value = []
    relatedContentCount.value = 0
    showRequestError(error, error?.message || '关联内容加载失败，请在表单设置中刷新')
  }
}
...
function syncNodeDataSourceBinding(field, { throwOnError = false } = {}) {
  ...
  try {
    ...
    showRequestError(error, error.message || '数据源映射格式不正确')
    return false
  }
}
...
async function validateFormActionsForPersistence({ requireEventBindings = false } = {}) {
  ...
  if (requireEventBindings && form.value.id) {
    try {
      ...
      showRequestError(error, '暂时无法校验按钮事件绑定，请稍后重试')
      return false
    }
  }
...
async function openRuntimeCode() {
  ...
  try {
    ...
    showRequestError(error, error?.message || '生成表单最终代码失败')
  } finally {
