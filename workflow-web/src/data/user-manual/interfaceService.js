...
export default {
  ...
  title: '接口服务用户手册',
  subtitle: '详细说明接口服务何时使用、如何创建、每个配置项的含义，以及如何查看服务引用并在对应设计器维护事件绑定。',
  version: '接口服务与使用情况收口版',
  ...
  sections: [
      ...
      topics: [
        {
          ...
          title: '进入页面',
          lead: '从左侧菜单进入“接口服务”。页面分为“接口服务”和“事件使用情况”两个页签。',
          blocks: [
            {
              type: 'steps',
              items: [
                { title: '打开接口服务页签', text: '先维护服务、操作、Schema、超时和缓存，并执行调试。' },
                { title: '打开事件使用情况页签', text: '选择接口服务，查看它被哪些实体、表单、列表、字段或按钮事件引用，以及草稿和发布状态。' },
                { title: '前往对象设计器配置', text: '使用引用行中的“前往配置”进入唯一编辑入口；保存绑定草稿后发布对应表单或列表。' }
              ]
            },
            {
              type: 'table',
    ...
    },
    {
      ...
      index: '07',
      title: '使用情况与事件绑定',
      summary: '接口服务页集中查看引用关系；事件执行链在实体、表单或列表的上下文中配置。',
      topics: [
        {
          ...
          blocks: [
            {
              ...
              columns: fieldColumns,
              rows: [
                { field: '接口服务', meaning: '当前查看引用关系的服务定义。', when: '排查服务影响范围、准备变更或下线前。', how: '按名称或编码选择服务。', effect: '总览只读，不在这里修改事件绑定。' },
                { field: '引用位置', meaning: '展示实体、表单、列表、字段或按钮及其稳定标识。', when: '确认绑定归属和作用范围。', how: '查看对象名称、配置层级和目标位置。', effect: '避免在错误页面或错误层级重复配置。' },
                { field: '事件与步骤', meaning: '展示触发事件、执行位置、步骤名称和接口操作。', when: '判断服务为什么以及何时被调用。', how: '按事件或操作筛选引用。', effect: '同一服务在不同事件中的调用分别列出。' },
                { field: '继承与最终生效', meaning: '展示引用来自实体默认还是页面覆盖，以及是否进入当前有效执行链。', when: '排查 INHERIT、REPLACE 或 DISABLE 带来的覆盖。', how: '查看继承来源和有效范围。', effect: '被下级替换的步骤不会被误认为仍然生效。' },
                { field: '草稿/已发布', meaning: '比较当前绑定草稿和页面激活发布版本。', when: '保存后页面行为没有变化。', how: '查看未发布、草稿已变更或已发布状态。', effect: '明确运行时仍读取哪个版本。' },
                { field: '前往配置', meaning: '进入该引用所属对象的上下文编辑入口。', when: '需要修改、停用或删除绑定。', how: '点击引用行中的“前往配置”。', effect: '表单、列表、字段和按钮不再在接口服务页重复编辑。' }
              ]
            }
          ]
        },
        {
          id: 'interface-service-binding-entry',
          title: '应该到哪里配置',
          blocks: [
            {
              type: 'table',
              columns: fieldColumns,
              rows: [
                { field: '实体默认事件', meaning: '为实体下的表单和列表提供可继承的默认执行链。', when: '多个页面复用同一事件逻辑。', how: '进入“实体配置 → 默认事件”。', effect: '相关表单或列表重新发布后使用新的实体默认链。' },
                { field: '表单事件', meaning: '配置具体表单的生命周期和数据事件。', when: '当前表单需要追加、替换或禁用实体默认逻辑。', how: '进入表单设计器的“表单设置 → 初始化与数据处理 → 表单事件”。', effect: '只影响当前表单。' },
                { field: '表单字段或按钮事件', meaning: '配置具体字段、子表单或表单按钮事件。', when: '联动回填或按钮需要调用服务。', how: '在表单设计器选中对应字段或按钮后打开事件绑定。', effect: '使用稳定字段编码或按钮编码定位。' },
                { field: '列表事件', meaning: '配置具体列表的加载、导出、数据操作或按钮事件。', when: '当前列表需要独立执行链。', how: '进入列表设计器并打开“事件绑定”。', effect: '只影响当前列表。' }
              ]
            }
          ]
        },
