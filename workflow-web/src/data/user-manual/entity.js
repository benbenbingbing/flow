...
export default {
  ...
  title: '实体配置用户手册',
  subtitle: '覆盖实体创建、字段与关系、数据权限、递归表单节点、列表单项配置、统一数据源、草稿发布和迁移兼容的完整配置闭环。',
  version: '当前 UI 配置基线',
  ...
  sections: [
    {
      ...
      topics: [
        ...
        },
        {
          ...
          title: '系统标准字段',
          lead: '动态实体创建后平台自动生成标准字段，实体设计器默认展示系统字段；独立实体隐藏不适用的流程字段，升级为流程实体时会幂等补齐并开放相关配置。',
          blocks: [
            {
              type: 'table',
              columns: [
              ...
              rows: [
                ...
                { field: 'deptId / 所属部门', meaning: '记录归属部门。', behavior: '创建实体时自动绑定平台“组织部门”系统实体（sys_organization），可直接单选组织，并参与本部门、本部门及子部门权限。' }
              ]
            },
            {
              type: 'callout',
              tone: 'info',
              title: '系统字段默认可见',
              text: '进入实体设计后系统字段默认展开，便于立即核对所属部门、状态、流程实例和提交人等平台字段。可以临时取消“系统字段”勾选缩短字段列表，但不会删除或停用这些字段。'
            }
          ]
        }
      ]
          ...
          blocks: [
            {
              ...
              columns: optionColumns,
              rows: [
                { option: '设计', meaning: '进入实体设计器，维护字段、关系、默认事件、附件、引用、编码和数据权限。', notes: '默认事件是表单和列表可继承的上级执行链；字段保存及发布限制保持不变。' },
                ...
                { option: '表单', meaning: '维护一个实体的多个表单、默认表单以及初始化与数据处理。', notes: '流程节点选择的表单被删除或禁用会影响运行。' },
          ...
          blocks: [
            {
              ...
              columns: fieldColumns,
              rows: [
                { field: '默认事件页签', meaning: '维护当前动态实体的上级事件执行链。', defaultLimit: '仅有实体维护权限的管理员可见；平台系统实体不可配置。', effect: '同一实体下的表单和列表可按事件编码继承。', publish: '保存后需重新发布相关表单或列表，运行时才读取新快照。' },
                { field: '继承并追加', meaning: '保留实体默认步骤并在页面层追加步骤。', defaultLimit: '表单或列表未配置同名事件时直接继承。', effect: '最终执行链同时包含实体级和页面级步骤。', publish: '以页面当前激活发布版本为准。' },
                { field: '替换上级 / 禁用自定义', meaning: '由具体表单或列表覆盖实体默认自定义步骤。', defaultLimit: '只能在对应页面设计器配置。', effect: '替换上级使用本级步骤；禁用自定义回到平台处理。', publish: '发布页面后生效，不会修改实体默认记录。' }
              ]
            },
            {
              type: 'callout',
          ...
          blocks: [
            {
              ...
              columns: fieldColumns,
              rows: [
                { field: '关联数量', meaning: '一条当前记录对应多少条关联记录。', defaultLimit: '新建默认一对一；一对一选择表单，一对多选择列表。', effect: '表单设计自动限定可选页面类型，运行时按关联字段查找数据。', publish: '保存和发布均检查关系与展示类型一致。' },
                { field: '关联实体', meaning: '要展示数据的目标实体。', defaultLimit: '必填；选择已发布的动态实体。', effect: '表单设计只提供此实体的已发布表单或列表。', publish: '先发布关联实体及目标表单或列表。' },
                { field: '关联字段', meaning: '位于关联实体中、保存当前记录 ID 的单值字段。', defaultLimit: '支持长度为 64～4096 的普通字符串字段，以及指向当前实体的单值引用；不支持数字、多值或引用其他实体的字段。', effect: '匹配规则为关联记录.关联字段 = 当前记录.id；引用字段优先显示，只有一项时自动选择。', publish: '不存在时先到关联实体新增文本或单值引用字段，保存并发布。' },
                { field: '内部标识', meaning: '关系编码及承载关系结果的数据键。', defaultLimit: '新建留空自动生成；不需要在任一实体创建同名字段；创建后保持稳定。', effect: '供表单、接口和发布快照识别同一条关系。', publish: '编辑保留原标识；数据键不得覆盖当前实体的普通字段。' },
                { field: '所有权 / 级联删除', meaning: '是否随当前记录生命周期管理关联记录。', defaultLimit: '新建默认普通关联、不级联删除；组成关系可单独开启级联删除。', effect: '普通关联不会随当前记录删除。', publish: '变更前应检查已有数据和业务生命周期。' }
              ]
            },
            {
              type: 'bullets',
              items: [
                '实体关系在“实体设计 → 实体关系”独立维护，保存后发布当前实体，不需要创建 SUB_FORM 承载字段。',
                '表单设计左侧“实体关系”中点击“选择表单”或“选择列表”，选定已发布页面后添加，画布和预览直接显示目标布局。',
                '发布当前表单后，运行时直接嵌入目标页面，按当前记录自动筛选关联数据。无关联记录时显示空状态，新记录先保存后再加载。',
                '快捷入口默认查看；其他位置和独立编辑等操作在“关联内容”中配置。关联内容的目标记录独立保存，不随当前表单提交；已有组成型子表单仍沿用原来的聚合提交。'
              ]
            }
          ]
        },
          ...
          blocks: [
            {
              ...
              tone: 'warning',
              title: '列表未绑定规则时默认拒绝',
              text: '实体设计只维护规则目录。把规则绑到哪个列表，在列表「访问范围」中设置。新实体会预置「本人创建或提交」「相关人」和「存在待办」三条目录规则，默认不绑定；新列表未绑定允许规则时默认拒绝全部数据，也可配置为仅本人数据。存量列表先进入可审计观察期；若确需全部可见，必须由管理员填写原因并显式确认。相关人与存在待办相互独立；已被指定、尚未办理的下一审批人不会进入相关人。'
            },
            {
              type: 'steps',
              items: [
                ...
                { title: '在列表绑定规则', text: '未绑定允许规则时执行 DENY_ALL、PERSONAL 或经管理员确认的 EXPLICIT_ALL 安全策略。TEAM 只看已参与记录，HAS_TODO 只看当前未完成待办，列表可单独或同时绑定。授予「绕过数据范围」后该用户将看到全部数据。' },
                ...
                { title: '列表绑定后生效', text: '规则目录只维护方案。在列表访问范围中绑定并保存后立即发布快照；运行时只读取激活版本。' }
              ]
            }
          ]
        },
          ...
          blocks: [
            {
              ...
              rows: [
                ...
                { option: '当前用户存在待办 HAS_TODO', meaning: '匹配 process_task 中当前用户未完成的待办。', notes: '办理人可见数据用这条。实体当前办理人字段不准，数据范围已不再提供该选项。' },
          ...
          blocks: [
            {
              ...
              items: [
                ...
                '至少使用普通用户、部门负责人、审批人和数据管理员验证；授予「绕过数据范围」后该用户会看到全部数据，勾选时会要求确认。super_admin 默认拥有该权限，不能替代普通角色测试。',
    ...
    },
    {
      ...
      index: '07',
      title: '表单管理',
      summary: '维护多个表单、稳定节点 ID、草稿、预览、发布版本、复制以及初始化与数据处理。',
      topics: [
        {
          id: 'entity-form-list',
          title: '表单列表与操作',
          blocks: [
            ...
            },
            {
              ...
              columns: optionColumns,
              rows: [
                { option: '设计', meaning: '进入递归节点表单设计器。', notes: '顶部通过“添加节点”和“表单设置”调整表单，“保存全部”保存全部草稿；节点属性抽屉内可单独“保存当前节点”；拖拽排序仍使用独立保存接口。' },
    ...
    },
    {
      ...
      topics: [
          ...
          blocks: [
            {
              ...
              rows: [
                ...
                { field: '节点属性页签', meaning: '属性按“基础与布局、数据校验、联动与事件、子页面、数据与扩展”组织。', defaultLimit: '所有配置项保持显示，不适用的配置置灰；已有兼容配置继续保留。', effect: '基础页维护默认值和条件状态；联动页直接维护值、选项和事件；子页面维护子表单、参数传递和子列表；数据与扩展维护引用选择、数据源和组件扩展。', publish: '分组和编辑方式不改变草稿、发布快照或运行时优先级。' },
          ...
          blocks: [
            {
              ...
              columns: optionColumns,
              rows: [
                { option: 'SECTION', meaning: '区块容器，可包含布局与内容节点。', notes: '从“添加容器节点 > 区块”创建，适合按业务主题分组，不绑定数据。' },
                ...
                { option: 'TEXT', meaning: '说明、提示、无数据展示或节标题节点。', notes: '“添加节”创建 textStyle=SECTION_TITLE 的竖线标题；普通说明仍由“说明文本”创建，内容不执行脚本。' },
          ...
          blocks: [
            {
              ...
              rows: [
                ...
                { option: 'FIELD', meaning: '可编辑显示标签、父容器、兼容组件、必填/只读/隐藏、默认值、占位、组件参数、类型兼容校验、模式权限、gridSpan、字段事件、节点扩展和受控数据源。', notes: '仅允许 FIELD_OPTIONS、FIELD_DEFAULT、FIELD_COMPUTE、AFTER_LOAD、BEFORE_SUBMIT；长度、格式和正则只对 STRING、TEXT 显示，数值范围只对 INTEGER、LONG、DECIMAL、DOUBLE 显示；组件切换会清理不兼容参数、规则和绑定。' },
                { option: 'SUB_FORM', meaning: '可编辑显示标签、父容器、子表布局、已发布子表单版本、gridSpan、节点扩展和受控行数据源，用于引用一个子实体表单。', notes: '展示位置由父容器决定；仅允许 SUBFORM_ROWS、AFTER_LOAD、BEFORE_SUBMIT，子实体、关系和外键绑定锁定。' },
                { option: 'REPEATER', meaning: '可编辑显示标签、父容器、明细布局、已发布子表单版本、gridSpan、节点扩展和受控行数据源，用于一对多重复明细。', notes: '展示位置由父容器决定；仅允许 SUBFORM_ROWS、AFTER_LOAD、BEFORE_SUBMIT，不显示 FIELD 的默认值、普通组件、字段校验、模式权限或字段事件。' },
            ...
            },
            {
              ...
              tone: 'info',
              title: '新增与编辑使用同一属性契约',
              text: '新增节点和编辑节点必须读取同一份 nodeType Schema，并经过同一套后端 PATCH/创建白名单。普通属性 PATCH 只提交 parentId、props、rules、数据源和扩展等当前类型允许修改的内容，不提交 nodeKey、nodeType、bindingType、bindingRef 或 legacyProps；同级排序必须调用排序接口。前端未展示的属性不能被请求绕过；历史节点中不属于当前类型的字段 props、rules、组件参数或数据源绑定，在编辑保存时从活动配置清除，必要的原始值仅保留到 legacyProps 等非活动兼容区，不参与预览、发布校验或运行时执行。'
            },
            {
              type: 'callout',
              tone: 'warning',
              title: '绑定和技术字段不可直接修改',
              text: 'id、nodeKey、revision、orderKey、发布快照版本、bindingType 和 bindingRef 始终只读。节点一旦绑定实体字段或实体关系，nodeType、fieldId、fieldCode、关系和子实体绑定一并锁定；如需改变数据语义，先新建目标节点并选择新字段或关系，再迁移显示标签、布局、兼容组件和允许的数据源配置，完成草稿预览与发布后再删除旧节点。不要直接改绑定让历史记录指向新字段；服务端白名单同样拒绝未知、不兼容或已锁定字段。'
            },
            {
              type: 'table',
              columns: fieldColumns,
        ...
        },
        {
          ...
          title: '表单中的子表单与引用',
          blocks: [{
              type: 'table', columns: fieldColumns,
              rows: [
                ...
                { field: '子表单输入参数', meaning: '子表单在“表单设置 → 初始化与数据处理 → 输入参数”声明可接收的参数编码、中文名称、类型、必填、默认值和说明。', defaultLimit: '参数默认只存在于运行时，不写入子实体；子表单通过 params.参数编码 读取。', effect: '为子表单的数据源、联动和事件提供稳定输入契约。', publish: '必填参数必须有父表单映射或默认值；参数类型和默认值必须通过 Schema 校验。' },
                { field: '运行参数', meaning: '父表单的子表单节点把父记录ID、父字段、受控运行上下文或固定值映射到子表单参数。', defaultLimit: '目标只能选择当前钉定子表单发布版本已声明的参数；关系外键不可配置。', effect: '新增行、已有行和父字段变化后，子表单均读取最新 params。', publish: '切换子表单发布版本后失效映射会标红，修复前不能发布。' },
          ...
          blocks: [
            {
              ...
              columns: optionColumns,
              rows: [
                { option: 'FORM_INIT', meaning: '仅在新增模式初始化业务对象。', notes: '编辑、查看和审批模式不执行；只填充映射目标，不得覆盖无权访问或只读字段。' },
          ...
          blocks: [
            ...
            },
            {
              ...
              rows: [
                ...
                { field: '内置类型 / 执行方式', meaning: '内置选择固定 key；自定义按执行方式选择目标或填写处理器、组件名。', defaultLimit: '打开关联内容选择已配置内容，无需填写执行器。', effect: '决定点击行为。', publish: '函数和组件模式的扩展须在目标环境注册。' },
                { field: '自定义模式', meaning: '自定义按钮可选择函数、组件、打开列表、打开表单、打开关联内容或业务接口。', defaultLimit: '打开关联内容需选择当前列表已配置且启用的弹窗、抽屉或页面；工具栏需选中一条记录，操作列使用当前行。', effect: '按钮名称、图标、样式、权限和适用条件在这里统一配置。关联内容面板不再自动生成入口，历史配置也需在按钮中绑定。', publish: '保存按钮并发布当前列表后生效；组件和回调必须在目标环境注册。' },
    ...
    },
    {
      ...
      index: '12',
      title: '发布、版本与迁移',
      summary: '把实体结构和表单/列表草稿转为不可变发布记录，支持差异、激活回滚与幂等迁移。',
      topics: [
        {
          ...
          blocks: [
            {
              ...
              rows: [
                ...
                { option: '/diff', meaning: '比较草稿与当前激活 release，按稳定 ID 展示新增、修改、移动和删除。', notes: '同时展示数据源、权限和嵌套校验结果。' },
