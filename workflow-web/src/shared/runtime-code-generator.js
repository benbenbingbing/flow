...
// 钉版接口可执行文档只供服务端运行时使用，其中可能包含 Provider 配置。
// 即使上游响应误带该字段，“等价代码”也不能把它复制进浏览器可见制品。
...
function normalizeValue(value, key = '') {
  ...
  Object.entries(value).forEach(([entryKey, entryValue]) => {
    if (SERVER_ONLY_SNAPSHOT_KEYS.has(entryKey)
...
function toEquivalentFormVue(configLabel, sourceLabel, definition) {
  return [
    ...
    '  releaseVersion: { type: Number, default: undefined },',
    ...
    'const labelPosition = computed(() => resolveFormLabelPosition(formDefinition))',
    'const labelWidth = computed(() => resolveFormLabelWidth(formDefinition))',
    ...
    '})',
    'const actionSlotKeys = computed(() => formNodes',
    '  .filter(node => String(node?.nodeType || \'\').toUpperCase() === \'ACTION_SLOT\')',
    '  .map(node => node.nodeKey)',
    '  .filter(Boolean)',
    ')',
    'const footerActions = computed(() => formActions.value.filter(action =>',
    ...
    '}',
    '',
    '/** ACTION_SLOT 动作只进入匹配的节点插槽，不能重复落到底部操作栏。 */',
    'function slotActions(slotKey) {',
    '  return formActions.value.filter(action =>',
    '    String(action.placement || \'\').toUpperCase() === \'ACTION_SLOT\'',
    '      && String(action.slotKey || \'\') === String(slotKey || \'\')',
    '  )',
    '}',
    '',
    '/** 为每次表单按钮点击生成服务端幂等 nonce；同一次传输重试复用该值。 */',
    'function createFormActionRequestId() {',
    '  const nonce = globalThis.crypto?.randomUUID?.()',
    '    || `${Date.now().toString(36)}_${Math.random().toString(36).slice(2)}`',
    '  return `form_action_${nonce}`.slice(0, 128)',
    '}',
    '',
    'async function handleFormAction(action) {',
    '  // runtimeKey 只隔离页面 loading；服务端事件绑定始终使用稳定按钮 key。',
    '  if (action.type === \'built-in\') return',
    ...
    '      releaseId: props.releaseId || formDefinition.runtimeReleaseId',
    '        || formDefinition.formReleaseId || undefined,',
    '      releaseVersion: props.releaseVersion',
    '        ?? formDefinition.runtimeReleaseVersion',
    '        ?? formDefinition.formReleaseVersion ?? undefined,',
    '      releaseResolutionToken:',
    '        props.releaseResolutionToken',
    '        || formDefinition.releaseResolutionToken || undefined,',
    '      entityCode: formDefinition.entityCode || \'\',',
    '      targetType: \'BUTTON\',',
    ...
    '        form: { ...formData }',
    '      },',
    '      // 可映射 context 不携带表单或身份坐标；服务端会从已发布快照和鉴权结果重建可信上下文。',
    '      context: {}',
    '    })',
    '  } finally {',
...
function toEquivalentListVue(configLabel, sourceLabel, definition) {
  return [
    ...
    '    releaseId: listDefinition.runtimeReleaseId',
    '      || listDefinition.releaseId || listDefinition.activeReleaseId || undefined,',
    '    releaseVersion: listDefinition.runtimeReleaseVersion',
    '      ?? listDefinition.publishedVersion ?? undefined,',
    '    releaseResolutionToken:',
    '      listDefinition.releaseResolutionToken || undefined,',
    '    entityCode,',
    '    listKey,',
    '    targetType: \'BUTTON\',',
    '    targetKey: String(actionKey(button)),',
    '    recordId: row?.id,',
    '    selectedIds: selectedRows.value.map(item => item.id).filter(Boolean),',
    '    input: {},',
    '    context: runtimeContext',
    '  })',
    '}',
