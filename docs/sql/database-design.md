...
文档版本：1.7；整理日期：2026-09-23。

本文按业务模块记录 Flow 平台的数据库结构、表间关系和使用情况。每张表单列章节，全部字段列在同一张表格中。

文档收录本机库当前的 133 张表、1,957 个字段，其中七张为历史保留表。`biz` 开头的业务表及其动态附属表、Flowable 引擎表不在范围内；平台自有 `process_*` 表正常收录。

结构已与 2026-09-23 本机 `localhost:3306/workflow` 核对，数据库已执行至 V102。表、字段、类型、可空性、默认值和索引以实际库为准，用途与关联结合 V001—V102 的 SQL、Java 迁移及当前源码说明。

字段表中的“NULL（隐式）”表示可空列没有显式 DEFAULT；JSON 格式要求由应用校验或表内 CHECK 约束承担。表间业务关联与物理外键分别说明。
...
- [8. UI扩展与发布](#8-ui扩展与发布)
  - [8.1 ui_event_binding 统一UI事件绑定表](#81-ui_event_binding-统一ui事件绑定表)
  - [8.2 ui_component_template UI组件模板目录表](#82-ui_component_template-ui组件模板目录表)
  - [8.3 ui_component_template_version UI组件模板版本表](#83-ui_component_template_version-ui组件模板版本表)
  - [8.4 ui_extension_definition 统一扩展定义目录表](#84-ui_extension_definition-统一扩展定义目录表)
  - [8.5 ui_config_release 表单列表发布快照表](#85-ui_config_release-表单列表发布快照表)
  - [8.6 ui_config_release_audit UI发布审计表](#86-ui_config_release_audit-ui发布审计表)
  - [8.7 ui_config_hotfix_target UI热修复目标快照表](#87-ui_config_hotfix_target-ui热修复目标快照表)
  - [8.8 ui_config_hotfix_request UI热修复发布与观察记录表](#88-ui_config_hotfix_request-ui热修复发布与观察记录表)
  - [8.9 ui_hotfix_observation_metric UI热修复观察指标表](#89-ui_hotfix_observation_metric-ui热修复观察指标表)
...
- [13. 字典文件与平台运维](#13-字典文件与平台运维)
  ...
  - [13.10 sys_global_setting 全局设置与个人偏好表](#1310-sys_global_setting-全局设置与个人偏好表)
  - [13.11 flyway_schema_history Flyway迁移历史表](#1311-flyway_schema_history-flyway迁移历史表)
...
- `entity_id` → [entity_definition](#11-entity_definition-实体定义表).`id`；两端物理类型不同。
- `active_release_id` → [ui_config_release](#85-ui_config_release-表单列表发布快照表).`id`。
...
legacy_props_document 保存无法识别或暂不适用的历史属性，供节点恢复时读取。同一表单内只对活动节点编码做唯一约束；逻辑删除节点的 active_node_key 为 NULL，允许同一编码多次删除后重新添加。
...
结构依据：[V001__business_schema.sql](../../workflow-server/workflow-db-migrator/src/main/resources/db/migration/V001__business_schema.sql)、[V093__entity_form_node_drop_legacy_key_unique.sql](../../workflow-server/workflow-db-migrator/src/main/resources/db/migration/V093__entity_form_node_drop_legacy_key_unique.sql)。
...
| `allowed_scenes` | 历史场景配置 | `longtext` | 是 | `NULL`（隐式） | 功能已移除，应用不再读写 | 停用 |
...
- `field_id` → [entity_field](#12-entity_field-实体字段定义表).`id`；两端物理类型不同。
- `interface_extension_id` → [ui_extension_definition](#84-ui_extension_definition-统一扩展定义目录表).`id`；由应用校验类型、状态、作用域和绑定用途，没有物理外键。
...
数据版本配置负责捕获规则与快照范围，记录版本表保存历史内容。`entity_mutation_receipt` 为实体写入和界面动作提供幂等控制。
...
| `process_instance_id` | 流程实例ID | `varchar(64)` | 是 | `NULL` | 流程实例ID。 | 现存 |
| `state` | 处理状态 | `varchar(20)` | 否 | 无 | 关联状态：PENDING 等待发起、ACTIVE 已关联运行实例、ENDED 已结束；与 end_type 配合区分生命周期和结束原因。 | 现存 |
...
| `end_type` | 流程结束类型 | `varchar(20)` | 是 | `NULL` | 在流程结束同步时记录 COMPLETED（正常结束）、TERMINATED（终止）或 WITHDRAWN（撤回），用于区分结束原因。关联状态统一为 ENDED，业务审批是否通过仍看业务状态，不能仅凭 COMPLETED 判断。未结束或历史原因未确认时可为空。 | 现存 |
...
process_instance_id 唯一约束防止同一流程实例重复关联；状态同步还使用幂等事件表。流程生命周期与业务审批状态分别管理：state 表示关联是否结束，end_type 记录如何结束，entity_status 保留业务状态。

#### 6.1.5 来源与迁移

结构依据：[V003__distributed_process_foundation.sql](../../workflow-server/workflow-db-migrator/src/main/resources/db/migration/V003__distributed_process_foundation.sql)、[V006__durable_process_status_sync.sql](../../workflow-server/workflow-db-migrator/src/main/resources/db/migration/V006__durable_process_status_sync.sql)、[V102__entity_process_lifecycle.java](../../workflow-server/workflow-db-migrator/src/main/java/db/migration/V102__entity_process_lifecycle.java)。
...
| `data_name` | 流程数据名称快照 | `varchar(500)` | 是 | `NULL` | 创建知会时保存业务数据当时的名称，供知会列表和通知展示。后续业务数据改名不更新此值；历史记录保持为空，不回查当前业务表补写。 | 现存 |
...
抄送知会与审批待办是不同事实，不表示接收者获得办理任务的权限。data_name 固定创建时的名称，避免历史知会随业务记录改名而改变。

#### 6.7.5 来源与迁移

结构依据：[V001__business_schema.sql](../../workflow-server/workflow-db-migrator/src/main/resources/db/migration/V001__business_schema.sql)、[V092__process_cc_name_snapshot.sql](../../workflow-server/workflow-db-migrator/src/main/resources/db/migration/V092__process_cc_name_snapshot.sql)。
...
### 8.1 ui_event_binding 统一UI事件绑定表
...
### 8.2 ui_component_template UI组件模板目录表
...
### 8.3 ui_component_template_version UI组件模板版本表
...
- `template_id` → [ui_component_template](#82-ui_component_template-ui组件模板目录表).`id`。
...
### 8.4 ui_extension_definition 统一扩展定义目录表
...
### 8.5 ui_config_release 表单列表发布快照表
...
### 8.6 ui_config_release_audit UI发布审计表
...
- `release_id` → [ui_config_release](#85-ui_config_release-表单列表发布快照表).`id`。
...
### 8.7 ui_config_hotfix_target UI热修复目标快照表
...
### 8.8 ui_config_hotfix_request UI热修复发布与观察记录表
...
### 8.9 ui_hotfix_observation_metric UI热修复观察指标表
...
- `request_id` → [ui_config_hotfix_request](#88-ui_config_hotfix_request-ui热修复发布与观察记录表).`id`。
...
| `deleted` | 逻辑删除标记 | `tinyint` | 否 | `'0'` | 逻辑删除标记。 | 现存 |
| `signature_status` | 来源验签状态 | `varchar(32)` | 否 | `'UNKNOWN'` | 导入时记录来源校验结论：VERIFIED 表示签名通过，MISMATCH_CONFIRMED 表示签名不一致但已人工确认来源，UNKNOWN 表示历史记录没有验签证据。与分析、发布状态分别保存，后续查看导入批次时据此说明包的来源可信程度。 | 现存 |
| `signature_confirmed_by` | 来源确认人 | `varchar(100)` | 是 | `NULL` | 签名不一致时，由服务端记录确认来源的导入账号；用于追溯谁接受了该配置包。签名通过的正常导入不填写。 | 现存 |
| `signature_confirmed_at` | 来源确认时间 | `datetime(6)` | 是 | `NULL` | 与来源确认人同时记录，保留人工确认的时间；后续重新分析配置差异不会覆盖这条确认记录。 | 现存 |
...
导入不是直接发布；条目比较、环境映射和发布状态分别记录。签名不一致时，用户必须确认当前包的校验和后才能继续导入；验签结论和确认记录独立保存，不随依赖分析报告覆盖。签名密钥由 sys_global_setting 的系统级设置管理，导入包表不保存密钥。

#### 12.5.5 来源与迁移

结构依据：[V001__business_schema.sql](../../workflow-server/workflow-db-migrator/src/main/resources/db/migration/V001__business_schema.sql)、[V091__migration_signing_global_setting.sql](../../workflow-server/workflow-db-migrator/src/main/resources/db/migration/V091__migration_signing_global_setting.sql)。
...
### 13.10 sys_global_setting 全局设置与个人偏好表

#### 13.10.1 业务说明

保存系统设置和用户个人偏好。一行表示某个作用域内的一项设置；新增设置项时注册设置键及使用逻辑，不需要增加数据库列。当前用于实体设计面板折叠、桌面主菜单折叠、顶部标签模式、移动端主题和配置迁移签名密钥。
...
| 字段名 | 中文名称 | 数据类型 | 允许空 | 数据库默认值 | 业务含义与约束 | 使用状态 |
| --- | --- | --- | --- | --- | --- | --- |
| `id` | 设置记录ID | `varchar(64)` | 否 | 无 | 由应用分配。修改、恢复默认时与 version 一起定位原记录；删除后重建使用新 ID，防止旧请求覆盖新设置。 | 现存 |
| `scope_type` | 设置作用域 | `varchar(16)` | 否 | 无 | SYSTEM 为系统值，USER 为个人覆盖值；服务端按注册规则限制某项设置允许使用的作用域。 | 现存 |
| `owner_id` | 设置归属 | `varchar(64)` | 否 | 无 | SYSTEM 固定为字符串 0；USER 保存 sys_user.id。个人接口从当前登录身份获取此值，不接受客户端指定其他用户。 | 现存 |
| `setting_key` | 稳定设置键 | `varchar(160)` | 否 | 无 | 程序按此键读取设置，例如 ui.layout.tabs_enabled。键须在 GlobalSettingRegistry 注册，单独向表中插入一个键不会自动增加功能。 | 现存 |
| `name` | 设置名称 | `varchar(100)` | 否 | 无 | 在管理界面展示该设置的用途。由注册定义提供，不能代替 setting_key 参与业务匹配。 | 现存 |
| `setting_value_type` | 设置值类型 | `varchar(16)` | 否 | 无 | BOOLEAN、NUMBER、STRING 或 JSON；后端据此校验值，前端据此选择输入控件。同一设置键的系统值和个人值必须使用注册类型。 | 现存 |
| `setting_value` | 设置值文本 | `text` | 否 | 无 | 应用按 JSON 格式序列化和解析，例如 true、20、双引号包裹的字符串或对象。不是数据库原生 JSON 列；拒绝顶层 null、类型不符及超过 16 KiB 的文本，合法的 false、0 和空字符串不能当成缺省。 | 现存 |
| `remark` | 生效规则说明 | `varchar(500)` | 是 | `NULL` | 展示取值含义、作用范围、继承关系和生效时机。说明本身不执行逻辑，实际行为由注册定义及设置使用方实现。 | 现存 |
| `version` | 乐观锁版本 | `bigint` | 否 | `0` | 更新、删除按 ID 和旧版本共同匹配，成功更新后递增；版本不符时拒绝覆盖，避免多个页面互相覆盖偏好。 | 现存 |
| `created_by` | 创建人 | `varchar(64)` | 是 | `NULL` | 创建设置记录的用户 ID，由服务端填写；迁移初始化等无操作用户的场景可为空。 | 现存 |
| `updated_by` | 最近修改人 | `varchar(64)` | 是 | `NULL` | 最近一次修改设置的用户 ID，用于定位维护人员；历史操作仍由系统审计记录。 | 现存 |
| `create_time` | 创建时间 | `datetime(6)` | 否 | `CURRENT_TIMESTAMP(6)` | 记录首次建立时间，修改设置值时保留。 | 现存 |
| `update_time` | 更新时间 | `datetime(6)` | 否 | `CURRENT_TIMESTAMP(6)` | 更新时自动刷新，用于查看最后维护时间；并发控制使用 version。 | 现存 |

#### 13.10.3 索引与关联

- ``PRIMARY KEY (`id`)``。
- ``UNIQUE KEY `uk_sys_global_setting_owner_key` (`scope_type`,`owner_id`,`setting_key`)``：同一归属下，每个设置键只有一条记录，也支持按用户批量读取偏好。
- `chk_sys_global_setting_scope_owner`：SYSTEM 的 owner_id 必须为 0；USER 的 owner_id 必须非空且不为 0。
- `chk_sys_global_setting_key`、`chk_sys_global_setting_name`：设置键和名称去除首尾空白后不能为空。
- `chk_sys_global_setting_value_type`：只允许 BOOLEAN、NUMBER、STRING、JSON 四种值类型。
- `chk_sys_global_setting_version`：版本号不得小于 0。

本表未声明物理外键。USER 记录的 owner_id 业务关联 sys_user.id，由服务端验证用户存在且有效；SYSTEM 使用专用归属标识 0。

#### 13.10.4 业务规则

允许个人覆盖的设置按“当前用户值 → 系统值 → 程序默认值”读取。恢复继承时物理删除对应记录，不把当前系统值复制成个人值；因此本表不设逻辑删除列。对象和数组按整项覆盖。

面板折叠、主菜单折叠及标签模式允许个人覆盖。移动端主题 ui.mobile.theme 只支持系统设置；标签模式只保存开关，已打开的标签和业务输入不写入本表。

配置迁移签名密钥 config.migration.signing_key 仅允许系统级维护，用于迁移包 HMAC-SHA256 签名与验签。初始化生成随机值，维护接口不回显已保存密钥，也不允许个人覆盖或通过删除恢复默认。更换密钥后，新签名和验签立即使用新值，已导出的包保留原签名。

#### 13.10.5 来源与迁移

结构依据：[V090__global_settings.sql](../../workflow-server/workflow-db-migrator/src/main/resources/db/migration/V090__global_settings.sql)。签名密钥、主菜单折叠和标签模式的初始设置分别见 [V091](../../workflow-server/workflow-db-migrator/src/main/resources/db/migration/V091__migration_signing_global_setting.sql)、[V100](../../workflow-server/workflow-db-migrator/src/main/resources/db/migration/V100__sidebar_collapsed_setting.sql)、[V101](../../workflow-server/workflow-db-migrator/src/main/resources/db/migration/V101__workspace_tabs_setting.sql)。移动端主题的类型和默认值由注册代码提供。

实现定位：[GlobalSettingRegistry.java](../../workflow-server/workflow-admin/src/main/java/com/workflow/admin/setting/application/GlobalSettingRegistry.java)、[GlobalSettingService.java](../../workflow-server/workflow-admin/src/main/java/com/workflow/admin/setting/application/GlobalSettingService.java)、[GlobalSettingMapper.java](../../workflow-server/workflow-admin/src/main/java/com/workflow/admin/setting/infrastructure/persistence/mapper/GlobalSettingMapper.java)。

### 13.11 flyway_schema_history Flyway迁移历史表
...
本次按本机库实际结构核对 133 张表、1,957 个字段，并检查字段类型、可空性、默认值、索引、目录锚点和源码链接。新增全局设置表说明，补齐迁移包验签、流程结束类型和知会名称快照字段，修正表单节点索引及列表场景字段类型。已删除的结构不再列为表章节。

索引清单结合正式迁移与实际库核对。表间业务关联不等于物理外键；历史保留表仍按本机实际结构登记。

本次仅更新设计文档，未执行数据库结构变更。
