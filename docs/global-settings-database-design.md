...
日期：2026-09-14；更新：2026-09-26。以下保留各阶段实现记录，当前三个界面偏好已合并为 JSON，见第 12 节。迁移在隔离测试库验证，未对现有业务库执行变更。
...
应按记录是否存在选择来源。合法的 `false`、`0` 或空字符串不能被当作“未设置”。普通对象/数组按整项覆盖，不做隐式深度合并；第 12 节的用户界面偏好显式采用字段级继承。
...
## 12. 用户界面偏好合并（2026-09-26）

三项可由用户自行切换的偏好统一注册为 `ui.user_preferences`，名称“用户界面偏好”，类型 `JSON`。仍使用 `sys_global_setting.setting_value` 的普通文本列，系统和每个有个人选择的用户各一行，不增加表或列。

```json
{
  "fieldTypesCollapsed": false,
  "sidebarCollapsed": false,
  "tabsEnabled": false
}
```

| 原设置键（V108 后停用） | JSON 字段 | 默认值 |
| --- | --- | --- |
| `ui.entity_design.field_types_collapsed` | `fieldTypesCollapsed` | `false`，字段类型面板展开 |
| `ui.layout.sidebar_collapsed` | `sidebarCollapsed` | `false`，桌面主菜单展开 |
| `ui.layout.tabs_enabled` | `tabsEnabled` | `false`，单页模式 |

- 系统和用户记录都允许稀疏对象。每个字段按 `USER → SYSTEM → 程序默认值` 继承，显式 `false` 是覆盖，缺失字段才表示继承。只接收上述三个布尔字段，拒绝未知字段、非布尔值和数组。
- 响应 `value` 包含合并后的三个有效值；新增 `overrideValue` 返回当前编辑作用域的实际稀疏对象，没有合法覆盖时为 `null`。`override` 继续表示当前作用域的记录 ID/版本，即使记录内容非法也保留。其他设置的 `overrideValue` 为 `null`，敏感设置仍不回显。
- 保存个人偏好使用同一个键及一份版本，基于 `overrideValue` 修改指定字段，不能把 `value` 整体保存为个人选择。恢复某个字段时只移除该属性；最后一个个人字段恢复时删除整行。
- `userInterfacePreferences` store 统一三个入口的读取、版本及串行保存队列。已有字段类型、侧栏和标签 store 仅作为字段访问入口。读取或保存失败不回退当前界面；下一次用户操作重新读取版本并合并明确修改的字段。账号变化立即停用旧队列。
- 全局设置页以一张“用户界面偏好”卡片展示三个开关，管理员修改和恢复默认后刷新当前账号有效值，已有个人字段保持优先。侧边栏标识和移动端主题仍为独立系统设置。

新增迁移 `workflow-server/workflow-db-migrator/src/main/java/db/migration/V108__merge_user_interface_preferences.java`。创建前检查 SQL 和 Java 迁移最大版本为 V107。V108 按作用域和归属聚合原三项记录，保留显式布尔值及字段缺失状态；已有新键同名字段优先，不预建其他用户记录。先验证全部待迁移数据，再在事务内写入 JSON 并按版本删除旧行；非法旧数据中止迁移并保留原记录。仅使用 JDBC 和应用 JSON 解析，不使用数据库 JSON 函数。

升级时停止旧版本应用写入设置，由独立迁移器执行至 V108，再启动新版后端及前端。旧三项键不再注册，旧页面需要刷新。业务库未执行此迁移；历史 V090、V100、V101 等迁移均未修改、删除或重命名。

## 13. 侧边栏标识内嵌图片（2026-09-26）

`ui.layout.sidebar_branding` 仍为系统级 JSON，新增可选 `imageBase64` 字符串，与名称和备用图标整体保存：
...
非空图片使用完整 Data URL，例如 `data:image/png;base64,...`。支持 PNG、JPEG、GIF 和 WebP，解码后的文件最大 32 KiB；校验 MIME、Base64 编码及文件头，不接受外部图片地址或 SVG。为容纳 Base64 编码，仅此设置的 JSON 文本上限为 48 KiB，其他设置仍为 16 KiB，现有 TEXT 列无需变更。

全局设置提供本地上传、更换、移除及预览，点击“保存标识”才提交。上传不经过文件服务；读取失败保留原图，刷新配置或移除图片时不会被旧异步读取覆盖。图片优先显示在原 24×24 图标位置，保持比例；桌面展开、收起和窄屏导航共用配置。图片无法解码时展示备用图标。

旧 JSON 没有 `imageBase64` 时按空字符串处理，原名称和图标继续有效。移除图片只清空该属性；恢复程序默认值则恢复原名称和 Connection 图标。本次图片扩展无新增、修改或删除迁移文件，前次 V108 保持不变。
