...
- 项目管理业务资产全部位于 `workflow-server/biz-project/`。
- 实体、字段、表单、列表、菜单、数据范围、节点审批和状态映射均通过配置实现。
- 项目专用运行时代码集中在 `biz-project`：F05 系统资产生效处理器，以及 F03/F06 的四个动作处理器和一个跨实体治理服务。
...
| 范围 | 文件 | 修改内容 | 发现方式与必要性 |
| --- | --- | --- | --- |
| 模块装配 | `workflow-server/pom.xml` | 注册 `biz-project` 模块和依赖版本 | 否则模块不参与聚合构建 |
| 运行时装配 | `workflow-server/workflow-app/pom.xml` | 引入 `biz-project` | 否则配置资源和流程动作处理器不进入应用 |
...
`workflow-server/biz-project/src/main/resources/project-config/packages/project-f01-f06-v2.wfpack`
...
```text
workflow-server/biz-project/src/main/java/com/workflow/biz/project/
```
...
2. 从 `workflow-app/pom.xml` 移除 `biz-project` 依赖。
