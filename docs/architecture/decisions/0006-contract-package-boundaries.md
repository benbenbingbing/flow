...
5. `FlowAction` 归属 `process.action`；实体变更与 UI 契约分别归属
   `entity.mutation` 与 `entity.ui`；流程办理人解析和开放流程身份解析分别归属
   `process.assignment` 与 `process.open`。
...
com.workflow.contracts.process.open.port
com.workflow.contracts.process.open.spi
...
com.workflow.contracts.entity.mutation.spi
