...
| --- | ---: | ---: | ---: | --- |
| 审批流程 | 13 | 5 | 5 | F01、F05 通过 R11；F03、F06 通过 R13；F07 通过 R14 发布和真实审批验收 |
| 流程实体 | 13 | 5 | 5 | 增加 `project_member_change_request`，已完成 JOIN、LEAVE 正常路径验收 |
| 普通实体 | 18 | 10 | 6 | E03、E05、E07、E08、E09、E10 已被真实流程创建、更新或用于代码门禁 |
| 业务实体总数 | 31 | 15 | 11 | 5 个流程实体和 6 个普通实体通过当前模拟范围验收 |
| 实体列表配置 | 至少 31 个默认列表 | 25 | 11 个实体被真实场景读取 | 共 25 张列表、36 个范围策略、40 个范围绑定；多账号隔离仍待专项验收 |
| 实体表单配置 | 基础 39 张 | 23 | 13 张流程实体表单随发布配置生效 | F07 使用 1 张支持四种运行模式的自定义表单 |
| 通用及专用流程动作 | 39 | 15 | 15 | F01/F03/F05/F06/F07 的校验、生效、节点、连线、全局审计和通知动作全部成功 |

当前 R14 完整发布包包含 15 个实体、5 个流程、322 个自定义字段、23 张表单、25 张列表、36 个数据范围策略、40 个范围绑定、20 个菜单、32 个用户任务、60 条状态映射、15 个流程动作和 20 个迁移资产。复杂流程还需增加专业评审表单或节点专属表单，预计最终表单数量为 45 至 50 张。表单数量允许在实施时按节点复用情况调整，但实体和流程数量按本节执行。
...
| F07 | `project_member_change_process` | 项目成员变更审批 | `project_member_change_request` | 项目成员 | `[x] R14 JOIN/LEAVE、自定义表单及四类动作已验收` |
...
实现状态基线日期为 **2026-07-30**。R14 完整包 `WFP-PROJECT-F01-F07-V3-20260730-R14` 已通过正常登录账号完成上传、分析和发布，累计包含 5 个流程、5 个流程实体和 10 个普通实体。F01、F03、F05、F06、F07 已在各自标注范围内完成真实数据验收；其中 F07 JOIN/LEAVE 分别完成 5 个审批任务，成员创建、退出、权限回收和项目经理角色移交均已生效。E01、E02、E04、E06 已发布但仍需专项验收，其余未配置资产保持 `[ ] 未实现`。
...
| 5 | P2 项目治理 | F07 项目成员变更 | F03、E08 | 加入、退出、暂停、恢复和权限回收可闭环 | `[x] JOIN/LEAVE 正常路径、角色移交和权限回收已验收；其余类型待专项分支验收` |
...
| --- | ---: | ---: | ---: | ---: | --- |
| 审批流程 | 13 | 5 | 5 | 38.46% | `[x] F01、F03、F05、F06、F07 阶段模拟场景通过；剩余 8 个流程未实现` |
| 流程实体 | 13 | 5 | 5 | 38.46% | `[x] 增加项目成员变更流程实体并完成真实审批验收` |
| 普通实体 | 18 | 10 | 6 | 33.33% | E03、E05、E07、E08、E09、E10 `[x]`；E01、E02、E04、E06 `[?]` |
| 实体列表配置 | 至少 31 | 25 | 11 个实体被实际读取 | - | 仍需多账号数据范围和导出专项验收 |
| 实体表单配置 | 基础 39 | 23 | 13 张流程实体表单已发布 | 33.33% | F07 自定义表单已覆盖新增、编辑、审批和查看 |
| 流程动作 | 39 | 15 | 15 | 38.46% | 十五条动作均已在当前验收范围内执行成功 |
...
```text
workflow-server/biz-project/src/main/resources/project-config/packages/project-f01-f05-v1.wfpack
```
...
以下规则需要跨实体聚合、幂等写入或批准后变更多个实体，当前配置表达不足，因此在 `biz-project` 中通过代码实现：
...
### 21.10 第三批 F07 项目成员变更实现明细

#### 21.10.1 实现范围与资产

F07 在 2026-07-30 纳入 R14 发布包。当前源码和目标环境包含：

| 类型 | 编码 | 数量或范围 | 状态 |
| --- | --- | --- | --- |
| 流程实体 | `project_member_change_request` | 35 个业务及审批字段 | `[x] 已发布` |
| 生效实体 | `project_member` | 加入、退出、暂停、恢复、投入调整的目标实体 | `[x] JOIN/LEAVE 已真实回写` |
| 交叉实体 | `project_role_assignment` | 成员退出时撤销原角色并创建接替角色 | `[x] 已完成 1 条项目经理角色移交` |
| 自定义表单 | `ProjectMemberChangeForm` | 新增、编辑、审批、查看四种模式 | `[x] 页面已验证` |
| 列表 | `all`、`my_pending`、`effective` | 台账、本人待办、已生效记录 | `[x] 已发布并读取` |
| 数据范围 | 创建人/办理人策略 | 2 个策略及对应绑定 | `[x] 单账号范围已验证` |
| 菜单 | 项目成员变更、我的成员变更待办、已生效成员变更 | 3 个菜单 | `[x] 已发布` |
| BPMN | `project_member_change_process` | 5 个任务、20 条顺序流、20 条图形边、63 个路径点 | `[x] 设计器实际渲染 20 条连线` |
| 流程动作 | 流程、节点、连线、全局审计和通知 | 6 条 | `[x] JOIN/LEAVE 全部执行成功` |

R14 发布包：

```text
workflow-server/biz-project/src/main/resources/project-config/packages/project-f01-f07-v3.wfpack
```
...
F07 必须在 F03 项目立项和 E08 项目成员定义完成后实施。其完成后才具备实施 F08 角色变更和 F09 项目基线的完整人员基础。
...
每个审批结果后均使用排他网关进入批准或驳回路径。权限、安全节点同时支持条件跳过；所有驳回路径汇总到统一驳回结束事件，最终批准进入生效结束事件。

#### 21.10.3 自定义表单逻辑控制

| 条件 | 显示或控制行为 | 后端复核 |
| --- | --- | --- |
| 未选择 `operation_type` | 只显示基础范围、变更说明和标准路径预览 | 禁止提交 |
| `operation_type=JOIN` | 显示加入人员、来源部门、人员类型、计划退出日期和新投入比例 | 校验项目状态、重复有效成员、跨项目投入不超过 100% |
| `operation_type=LEAVE` | 显示目标成员、权限回收和工作角色交接 | 校验成员属于项目、有效角色、唯一负责人和交接人 |
| `operation_type=SUSPEND` | 显示目标成员、暂停原因和权限处理 | 校验当前成员必须为 `ACTIVE` |
| `operation_type=RESUME` | 显示目标成员和恢复说明 | 校验当前成员必须为 `SUSPENDED` |
| `operation_type=ALLOCATION_CHANGE` | 显示目标成员、新投入比例和跨项目投入仪表 | 校验新比例大于 0 且合计不超过 100% |
| 用工类型为外包或供应商 | 计划退出日期必填 | 启动动作重复校验 |
| 申请账号或环境访问 | `access_review_required_flag=true`，进入系统负责人节点 | 流程启动时写入实时流程变量 |
| 环境范围包含生产或选择敏感权限 | `security_review_required_flag=true`，进入安全节点 | 流程启动时写入实时流程变量 |
| 退出成员存在有效角色或账号权限 | `handover_required_flag=true`，交接成员和说明必填 | 禁止向非本项目有效成员交接 |
| 查看或审批模式 | 申请字段只读，保留审批路径、交叉数据和生效结果 | 由节点字段权限和对象权限共同控制 |

前端表单只负责交互、路径预判和即时提示。项目状态、成员状态、投入比例、有效角色、唯一负责人、权限回收和交接关系均由后端动作再次校验。

#### 21.10.4 数据关系与生效规则

1. 一个 `project_member_change_request` 必须关联一个 `project`。
2. JOIN 通过 `target_user_id` 创建一条当前有效 `project_member`，同一项目和用户不得重复有效。
3. LEAVE、SUSPEND、RESUME、ALLOCATION_CHANGE 必须通过 `project_member_id` 定位目标成员，目标成员的 `project_id` 必须与申请一致。
4. LEAVE 将成员状态改为 `LEFT`、投入比例改为 0、写入实际退出日期并标记权限已回收。
5. LEAVE 存在有效角色时，原 `project_role_assignment` 进入 `REVOKED`，并按交接信息幂等创建接替角色记录。
6. 接替成员必须是同项目有效成员；项目经理等唯一主负责人不得在没有接替人的情况下退出。
7. 申请实体保留来源成员快照、角色快照、权限快照、生效结果和错误信息，不物理删除历史申请。

#### 21.10.5 流程动作

| 动作处理器 | 作用域与时机 | 执行模式 | 业务作用 |
| --- | --- | --- | --- |
| `validateProjectMemberChangeHandler` | 流程启动 | `IN_TRANSACTION` | 执行跨实体门禁并写入权限、安全、交接实时路由变量 |
| `captureProjectMemberManagerReviewHandler` | 项目经理节点完成 | `IN_TRANSACTION` | 记录复核时间、操作人和申请编号 |
| `recordProjectMemberDecisionHandler` | `flow_pmo_approve` 连线经过 | `IN_TRANSACTION` | 记录最终批准连线、来源节点、目标节点和决策编码 |
| `applyProjectMemberChangeHandler` | 流程完成 | `AFTER_COMMIT` | 幂等创建或更新成员，撤销并移交有效角色 |
| `projectLifecycleAuditHandler` | 流程完成的全局动作 | `AFTER_COMMIT` | 记录统一项目生命周期审计摘要 |
| `sendNotificationHandler` | 流程完成 | `AFTER_COMMIT` | 发送成员变更结果站内通知 |

前四个处理器仅对 `project_member_change_request` 实体可见；生命周期审计处理器为全局动作。流程配置负责绑定时机、作用域、失败策略和重试参数，代码只实现配置无法表达的跨实体聚合与幂等写入。

#### 21.10.6 真实场景验收

模拟项目为 `PRJ2026073000006`，连续完成一条高权限 JOIN 和一条关键成员 LEAVE：

| 验收项 | 实际结果 |
| --- | --- |
| JOIN 流程实例 | `bddcf10d-8bb9-11f1-80b8-7612d5b69a47` |
| JOIN 审批 | 项目经理、部门负责人、系统负责人、安全负责人、PMO 共 5 个任务完成 |
| JOIN 生效 | 创建有效成员，投入比例 25%，权限和安全条件审批均执行 |
| LEAVE 流程实例 | `be82315e-8bb9-11f1-80b8-7612d5b69a47` |
| LEAVE 审批 | 同样完成 5 个任务，交接条件为必需 |
| LEAVE 生效 | 成员状态变为 `LEFT`，原项目经理角色撤销，向接替成员创建 1 条新角色 |
| BPMN 图形 | 20 条顺序流对应 20 条 `BPMNEdge`，共 63 个 `waypoint`，浏览器实际检测 20 条可见连接 |
| 自定义表单 | 新增及完成态详情正常渲染；JOIN/LEAVE 条件区块切换正确；审批路径预览正确；字段标题和关联记录均使用中文业务名称 |
| 动作 | 两个流程的启动、节点、连线、生效、全局审计和通知动作全部 `SUCCESS` |
| 证据文件 | `docs/project-member-change-e2e/project-member-change-20260730015454.json` |

#### 21.10.7 本轮发现并修复的平台缺陷

1. **启动动作不能更新实时流程变量**：动作上下文只能返回结果，条件网关读取不到动态计算的权限、安全和交接标记。现已为流程动作运行时增加受控的流程变量写入能力。
2. **迁移重新分析无法清除旧错误**：依赖已经补齐后，MyBatis 更新会跳过 `NULL`，导致旧 `error_message` 残留并继续阻断发布。现已增加显式 SQL 更新，允许清空包和资产错误。
3. **UI 事件运行时缺少访问策略**：打开自定义表单时，`FORM_OPEN` 被端点授权拦截器拒绝。现已为运行时事件接口增加受认证且包含对象级授权的访问声明。
4. **成员详情调用缺少实体上下文**：自定义表单原先使用不完整的通用 ID 地址读取成员。现改为 `project_member` 标准详情接口，复用实体对象权限和列表范围。
5. **通用审批详情误判附件并中断渲染**：无节点表单的历史待办会把任意对象字段当成附件组，并对布尔值或嵌套对象执行字符串 `split()`，导致审批弹窗无法打开。现仅把受支持的文件 URL 识别为附件，其余数组、对象和布尔值统一安全格式化为只读文本，并补充回归测试。
6. **完成态流程把网关误判为最后表单节点**：流程结束后进度接口原先直接取最后一个已完成节点，可能命中排他网关，导致完成态详情无法解析节点表单。现根据 BPMN 用户任务集合选择最后完成的用户任务，并在历史节点没有表单绑定时回退到实体默认表单。
7. **完成态审批详情退回英文字段编码**：审批弹窗在进度接口没有返回可渲染表单时直接使用通用详情，且没有实体字段定义可用于标题转换。现统一解析节点表单与默认表单、透传实体定义和字段元数据，通用详情也按实体字段中文名称展示。
8. **自定义实体关联记录只显示 ID 或“未命名记录”**：选择器回显阶段忽略运行时实体编码，后端批量查询又只读取 DTO 的自定义 `data`，丢失标准 `name`、`code` 和 `title`。现前端统一解析有效实体编码，后端从完整 DTO 生成回显数据，并为名称、标题、编码、业务编号和 ID 建立顺序兜底。

#### 21.10.8 当前边界与后续流程

F07 当前可标记为“阶段模拟场景 `[x]`”，不能据此宣称 13 个项目生命周期流程全部完成。仍需继续实现的 8 个流程按依赖顺序为：
...
F07 自身还需补充 SUSPEND、RESUME、ALLOCATION_CHANGE、驳回、条件跳过、多账号职责分离、动作失败重试和重复执行专项验收，完成后才能升级为生产级 `[x]`。
