...
@Component
@Slf4j
public class ZDWCreateUiDataSourceProvider implements UiDataSourceProvider {
    /**
    ...
    public String getCode() {
        return "ZDW-CREATE—UI_DATA_SOURCE_PROVIDER";
    }
    ...
    public String getDisplayName() {
        return "CREATE周大伟自定义数据源";
    }

    /**
     * 执行{@code zdw}创建界面数据来源提供者，并将结果传给后续步骤。
     *
     * @param context 执行上下文，向后续{@code zdw}创建界面数据来源提供者步骤传递身份、配置或状态
     * @param dataScopePlan 数据作用域方案，供本方法执行{@code zdw}创建界面数据来源提供者时使用
     * @param configuration 配置内容，决定后续{@code zdw}创建界面数据来源提供者的处理规则
     * @param input 待执行{@code zdw}创建界面数据来源提供者的原始输入，结果供调用方继续使用
     * @return 执行后的{@code zdw}创建界面数据来源提供者结果，供调用方继续处理
     */
    @Override
