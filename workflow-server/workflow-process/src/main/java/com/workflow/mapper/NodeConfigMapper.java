...
public interface NodeConfigMapper extends BaseMapper<NodeConfig> {
     ...
     * 根据流程配置ID查询节点列表
     */
    @Select("SELECT * FROM process_node_config WHERE process_config_id = #{processConfigId} ORDER BY create_time ASC")
    List<NodeConfig> findByProcessConfigId(@Param("processConfigId") String processConfigId);

    /**
     * 根据流程配置ID删除节点
     */
    @Delete("DELETE FROM process_node_config WHERE process_config_id = #{processConfigId}")
    void deleteByProcessConfigId(@Param("processConfigId") String processConfigId);
    
    /**
     * 根据节点ID和流程配置ID查询节点配置
     */
    @Select("SELECT * FROM process_node_config WHERE node_id = #{nodeId} AND process_config_id = #{processConfigId} LIMIT 1")
