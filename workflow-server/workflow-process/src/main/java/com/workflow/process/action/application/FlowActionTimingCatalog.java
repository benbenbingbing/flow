...
public class FlowActionTimingCatalog {
     ...
     * @return 去重后的触发时机选项列表
     */
    public List<FlowActionTimingOption> list(String scopeType, String bpmnType) {
        ...
        List<FlowActionTimingOption> options = new ArrayList<>();
        ...
        for (FlowActionTriggerTiming timing : FlowActionTriggerTiming.values()) {
            ...
            options.add(new FlowActionTimingOption(
                    timing.name(),
                    timing.getLabel(),
     ...
     */
    public Optional<FlowActionTimingOption> find(String code) {
        ...
        for (FlowActionTriggerTiming timing : FlowActionTriggerTiming.values()) {
            if (timing.name().equals(normalized)) {
                FlowActionScopeType optionScope = timing.getScopeTypes().iterator().next();
                return Optional.of(new FlowActionTimingOption(
                        timing.name(),
                        timing.getLabel(),
                        timing.getDescription(),
                        optionScope.name(),
