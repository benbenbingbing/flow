package com.workflow.admin.dictionary.infrastructure.persistence.mapper;

import com.baomidou.mybatisplus.core.toolkit.Wrappers;
import com.baomidou.mybatisplus.core.mapper.BaseMapper;
import com.workflow.admin.dictionary.infrastructure.persistence.record.SysDict;
import org.apache.ibatis.annotations.Mapper;

/**
 * 字典类型 Mapper
 */
@Mapper
public interface SysDictMapper extends BaseMapper<SysDict> {

    /**
     * 检查字典编码是否存在
     *
     * @param dictCode  字典编码
     * @param excludeId 排除的ID（更新时传入自身ID，新增传空串或 null）
     * @return 存在返回 true，否则 false
     */
    default boolean existsDictCode(String dictCode, String excludeId) {
        return selectCount(Wrappers.<SysDict>lambdaQuery()
                .eq(SysDict::getDictCode, dictCode)
                .ne(excludeId != null && !excludeId.isEmpty(), SysDict::getId, excludeId)) > 0;
    }
}
