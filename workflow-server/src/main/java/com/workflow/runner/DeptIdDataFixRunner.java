...
public class DeptIdDataFixRunner implements CommandLineRunner {
    ...
    private void fixDeptId(Connection connection) throws SQLException {
        ...
        try (Statement stmt = connection.createStatement();
             ResultSet rs = stmt.executeQuery("SELECT entity_code FROM entity_definition")) {
            while (rs.next()) {
        ...
        log.info("[DeptIdFix] 发现 {} 个实体定义，开始处理 dept_id ...", entityCodes.size());

        int totalUpdated = 0;
        for (String entityCode : entityCodes) {
            String tableName = "entity_data_" + entityCode;
