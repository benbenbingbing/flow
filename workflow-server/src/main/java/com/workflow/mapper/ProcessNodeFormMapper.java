...
public interface ProcessNodeFormMapper extends BaseMapper<ProcessNodeForm> {
     ...
     * 查询流程的节点表单绑定
     */
    @Select("SELECT * FROM process_node_form WHERE process_config_id = #{processConfigId}")
    List<ProcessNodeForm> selectByProcessConfigId(@Param("processConfigId") String processConfigId);
    
    /**
     * 查询节点的表单绑定
     */
    @Select("SELECT * FROM process_node_form WHERE process_config_id = #{processConfigId} AND node_id = #{nodeId} LIMIT 1")
