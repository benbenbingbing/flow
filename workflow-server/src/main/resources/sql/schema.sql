...
CREATE TABLE IF NOT EXISTS entity_field (
    ...
    db_column_name VARCHAR(100) COMMENT '数据库列名（下划线命名）',
    field_precision INT COMMENT '小数位数（精度），用于 DECIMAL 类型',
    ref_entity_id VARCHAR(64) COMMENT '关联实体ID（用于子表单/实体选择）',
    ref_entity_type VARCHAR(20) COMMENT '引用实体类型（CUSTOM/USER/DEPT/ROLE/GROUP）',
    display_mode VARCHAR(20) COMMENT '显示方式：embedded-嵌入, tab-Tab页（用于子表单）',
    ref_field_code VARCHAR(100) COMMENT '关联字段编码（用于子表单数据关联）',
    file_types VARCHAR(500) COMMENT '文件类型限制（用于附件类型）',
