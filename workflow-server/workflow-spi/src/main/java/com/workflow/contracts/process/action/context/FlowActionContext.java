 ...
 * {@link FlowActionProvider#execute(FlowActionContext)}。
...
public class FlowActionContext {
     ...
     * 内部查询辅助器，不对外序列化
     */
    private transient FlowActionRuntimePort runtimeAccess;
    ...
    /**
     * 整理可变流程变量快照数据，供调用方遍历或继续处理。
     *
     * @return 可变流程变量快照键值结果，供调用方继续处理
     */
     ...
     * 获取当前实体数据。
     *
     * @return 实体业务数据投影，不包含宿主实现类型
     */
    public EntityRecordData getEntityData() {
