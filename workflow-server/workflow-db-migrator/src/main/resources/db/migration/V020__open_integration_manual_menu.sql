INSERT INTO sys_menu (
  ...
  'Link',
  3,
  '/manual/open-integration',
  'manual/OpenIntegrationManual',
  'user-manual:open-integration:view',
  '0',
  '0',
  '0',
  '0',
  ...
  '0',
  '1',
  '开放集成应用、Open API、Webhook、Secret 与 HTTP Connector 使用手册',
  0,
  NULL,
  CURRENT_TIMESTAMP,
  NULL,
...
SELECT
  MD5(CONCAT(role_id, ':user_manual_open_integration_001')),
  role_id,
  'user_manual_open_integration_001',
  CURRENT_TIMESTAMP
...
WHERE parent_grant.menu_id = 'user_manual_dir_001'
  AND NOT EXISTS (
    ...
    WHERE existing_grant.role_id = parent_grant.role_id
      AND existing_grant.menu_id = 'user_manual_open_integration_001'
  );
