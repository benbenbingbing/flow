...
class UiAvailableInterfaceServiceTest {

    private UiExtensionDefinitionMapper sourceMapper;
    ...
    private UiAvailableInterfaceService service;

    @BeforeEach
    void setUp() {
        sourceMapper = mock(UiExtensionDefinitionMapper.class);
        ...
        service = new UiAvailableInterfaceService(
                sourceMapper,
                definitionMapper,
                formMapper,
                listMapper,
    ...
    void filtersFormOperationsByScopeContextKindAndSchema() throws Exception {
        ...
        List<UiAvailableInterface> result = service.available(
                "FORM",
                "form-a",
                "FIELD_OPTIONS");
        ...
        assertEquals(
                ...
                result.stream()
                        .map(UiAvailableInterface::extensionId)
                        .toList());
    }
    ...
    private UiExtensionDefinition definition(
        ...
        value.setDeleted(0);
        value.setProviderOperationCode(String.valueOf(operation.get("code")));
        value.setInterfaceContextType(String.valueOf(
                operation.get("contextType")));
        value.setInterfaceKind(String.valueOf(operation.get("kind")));
        value.setInputSchemaDocument(objectMapper.writeValueAsString(
                operation.getOrDefault("inputSchema", Map.of())));
        value.setOutputSchemaDocument(objectMapper.writeValueAsString(
                operation.getOrDefault("outputSchema", Map.of())));
        return value;
    }
