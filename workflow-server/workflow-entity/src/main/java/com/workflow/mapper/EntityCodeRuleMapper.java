...
public interface EntityCodeRuleMapper extends BaseMapper<EntityCodeRule> {
     ...
     */
    @Update("UPDATE entity_code_rule SET current_seq = #{newSeq}, seq_date = #{newDate}, update_time = NOW() " +
     ...
     * 更新当前序列号（同一天内递增）
     */
    @Update("UPDATE entity_code_rule SET current_seq = #{newSeq}, update_time = NOW() " +
