...
public interface EntityFlowStatusMappingMapper extends BaseMapper<EntityFlowStatusMapping> {
     ...
     * 根据流程配置ID查询状态映射
     */
    @Select("SELECT * FROM process_entity_status_mapping WHERE process_config_id = #{processConfigId} AND deleted = 0 ORDER BY sort_order")
    List<EntityFlowStatusMapping> findByProcessConfigId(@Param("processConfigId") String processConfigId);
    
    /**
     * 根据流程标识查询状态映射
     */
    @Select("SELECT * FROM process_entity_status_mapping WHERE process_key = #{processKey} AND deleted = 0 ORDER BY sort_order")
    List<EntityFlowStatusMapping> findByProcessKey(@Param("processKey") String processKey);
    
    /**
     * 根据流程配置ID和源节点查询
     */
    @Select("SELECT * FROM process_entity_status_mapping WHERE process_config_id = #{processConfigId} AND source_node_id = #{sourceNodeId} AND deleted = 0")
     ...
     * 根据流程配置ID、源节点和目标节点查询
     */
    @Select("SELECT * FROM process_entity_status_mapping WHERE process_config_id = #{processConfigId} AND source_node_id = #{sourceNodeId} AND target_node_id = #{targetNodeId} AND deleted = 0 LIMIT 1")
     ...
     * 根据实体编码查询
     */
    @Select("SELECT * FROM process_entity_status_mapping WHERE entity_code = #{entityCode} AND deleted = 0 ORDER BY sort_order")
