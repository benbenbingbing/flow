...
public interface EntityListScopeBindingMapper extends BaseMapper<EntityListScopeBinding> {
     ...
     * @return 数据范围绑定列表
     */
    default List<EntityListScopeBinding> findByEntityCode(String entityCode) {
        return selectList(Wrappers.<EntityListScopeBinding>lambdaQuery()
                .eq(EntityListScopeBinding::getEntityCode, entityCode)
                .orderByAsc(EntityListScopeBinding::getCreatedAt));
    }
