 ...
 * @param viewId 视图ID，后续用于处理嵌入式运行时发布版本行时定位或关联目标
 * @param viewKey 视图键，后续用于授权校验、关联或幂等去重
 * @param viewName 视图名称，后续用于处理嵌入式运行时发布版本行时匹配或展示
 * @param revision 修订版本，保存在对象中供后续校验、查询或展示
 * @param surfaceType 界面类型标识，决定后续嵌入式运行时发布版本行采用的处理分支
 * @param entityCode 实体编码，用于限定后续数据读取、校验或写入的实体范围
 * @param listKey 列表配置键，后续用于确定数据权限与展示字段范围
 * @param listReleaseId 列表发布版本ID，后续用于处理嵌入式运行时发布版本行时定位或关联目标
 * @param listReleaseVersion 列表发布版本，保存在对象中供后续校验、查询或展示
 * @param formReleaseId 表单发布版本ID，后续用于处理嵌入式运行时发布版本行时定位或关联目标
 ...
 * @param actorDisplayName 操作人展示名称，后续用于处理嵌入式运行时发布版本行时匹配或展示
