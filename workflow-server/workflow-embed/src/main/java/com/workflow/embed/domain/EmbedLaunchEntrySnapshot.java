 ...
 *
 * @param launchId 启动记录ID，后续用于处理嵌入式启动记录入口快照时定位或关联目标
 * @param parentOrigin 父级来源，保存在对象中供后续校验、查询或展示
 * @param channelId 通道ID，后续用于处理嵌入式启动记录入口快照时定位或关联目标
 * @param launchStatus 启动记录状态标识，决定后续嵌入式启动记录入口快照采用的处理分支
 ...
 * @param applicationStatus 应用状态标识，决定后续嵌入式启动记录入口快照采用的处理分支
 ...
 * @param grantStatus 授权状态标识，决定后续嵌入式启动记录入口快照采用的处理分支
