...
final class PinnedHttpResources implements AutoCloseable {
    ...
    synchronized Admission enter(URI uri) throws HttpTransportUnavailableException {
        if (closed || active >= maxConcurrent) throw new HttpTransportUnavailableException("HTTP 全局并发已达上限或已关闭");
        ...
        if (target == null) {
            ...
            if (targets.size() >= maxPools * 2) {
                ...
                if (targets.size() >= maxPools * 2) throw new HttpTransportUnavailableException("HTTP 目标状态容量已达上限");
            }
            target = new Target();
            targets.put(key, target);
        }
        if (target.active >= perTarget) throw new HttpTransportUnavailableException("HTTP 目标并发已达上限");
        boolean probe = target.openUntil != 0;
        if (probe && (now < target.openUntil || target.active > 0)) {
            throw new HttpTransportUnavailableException("HTTP 目标暂时熔断");
        }
        target.active++;
        active++;
    ...
    synchronized Lease lease(ApprovedEndpoint endpoint, HttpTransportRequest request, boolean privateAllowed)
            throws HttpTransportUnavailableException {
        if (closed) throw new HttpTransportUnavailableException("HTTP 连接池已关闭");
        ...
        if (pool == null) {
            if (pools.size() >= maxPools) {
                ...
                if (pools.size() >= maxPools) throw new HttpTransportUnavailableException("HTTP 连接池容量已达上限");
            }
